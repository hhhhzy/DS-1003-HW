...
Answer:
$f^*(x) = a_0 + a_1 x + a_2 x^2$ is an obvious Bayes predictor. Here we know that the joint distribution $P_{\mathcal{X} \times \mathcal{Y}}$ is $y = g(x) = a_0 + a_1 x + a_2 x^2$, so when $f^*(x) = g(x)$, the risk $R(f^*) = E[l(f(x),y]$ would reaches its minimum 0.
